...
Конвейер проверен на реальной генерации на графическом процессоре: для тестового снимка приняты все восемь запрошенных вариантов (по два на каждый из четырех сезонов), причем для зимы отбор честно отбраковал две попытки и добрал корректные с новыми начальными значениями. Полученный файл параметров сохранен в результатах как образец выходного формата. Тот же конвейер работает и полностью без доступа в интернет: модели один раз переносятся на сервер, после чего запуск идет из локального кеша - это проверено отдельным прогоном.
